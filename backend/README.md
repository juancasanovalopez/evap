# evap backend (Go, serverless en AWS)

API + páginas SSR en Go, empaquetadas como una única AWS Lambda detrás de
...
Ver [infra/](../infra) para Terraform (Lambda, API Gateway HTTP API,
DynamoDB, IAM de mínimo privilegio, SSM y el rol OIDC para GitHub Actions).
Los secretos OAuth/JWT se cargan como placeholders (`CHANGEME`) y deben
actualizarse manualmente tras el primer `apply`:
