...
const (
	...
	DashboardSensorGuide          = "dashboard.sensor_guide"
	...
	DashboardReadingsTitle        = "dashboard.readings_title"
	DashboardReadingsHelp         = "dashboard.readings_help"
	DashboardReadingsDevice       = "dashboard.readings_device"
	DashboardReadingsTemperature  = "dashboard.readings_temperature"
	DashboardReadingsTimestamp    = "dashboard.readings_timestamp"
	DashboardReadingsEmpty        = "dashboard.readings_empty"
	DashboardReadingsError        = "dashboard.readings_error"

	SensorGuideTitle       = "sensor_guide.title"
	SensorGuideIntro       = "sensor_guide.intro"
	SensorGuideConnection  = "sensor_guide.connection"
	SensorGuideEndpoint    = "sensor_guide.endpoint"
	SensorGuidePort        = "sensor_guide.port"
	SensorGuideProtocol    = "sensor_guide.protocol"
	SensorGuideTopic       = "sensor_guide.topic"
	SensorGuideAuth        = "sensor_guide.auth"
	SensorGuideUsername    = "sensor_guide.username"
	SensorGuidePassword    = "sensor_guide.password"
	SensorGuideInterval    = "sensor_guide.interval"
	SensorGuidePayload     = "sensor_guide.payload"
	SensorGuidePayloadText = "sensor_guide.payload_text"
	SensorGuideSteps       = "sensor_guide.steps"
	SensorGuideStepOne     = "sensor_guide.step_one"
	SensorGuideStepTwo     = "sensor_guide.step_two"
	SensorGuideStepThree   = "sensor_guide.step_three"
	SensorGuideBack        = "sensor_guide.back"

	SensorGuideDiagnosticsTitle    = "sensor_guide.diagnostics_title"
	SensorGuideDiagnosticsHelp     = "sensor_guide.diagnostics_help"
	SensorGuideSensorsTitle        = "sensor_guide.sensors_title"
	SensorGuideSensorsEmpty        = "sensor_guide.sensors_empty"
	SensorGuideSensorsError        = "sensor_guide.sensors_error"
	SensorGuideMessagesTitle       = "sensor_guide.messages_title"
	SensorGuideMessagesHelp        = "sensor_guide.messages_help"
	SensorGuideMessagesEmpty       = "sensor_guide.messages_empty"
	SensorGuideMessagesError       = "sensor_guide.messages_error"
	SensorGuideMessagesDevice      = "sensor_guide.messages_device"
	SensorGuideMessagesTemperature = "sensor_guide.messages_temperature"
	SensorGuideMessagesTimestamp   = "sensor_guide.messages_timestamp"
)
...
var englishMessages = map[string]string{
	...
	DashboardTitle: "Evaporation simulator", DashboardGreeting: "Hello, %s", DashboardProfile: "profile", DashboardSensorGuide: "MQTT", DashboardLogout: "Sign out", DashboardSystemStatus: "System status", DashboardSimulation: "Simulation", DashboardTotalLoss: "Total loss", DashboardVolume: "Volume", DashboardLoweredMeters: "Lowered meters", DashboardLocation: "Location", DashboardLocationHelp: "Click the map, drag the marker, or use your current location.", DashboardMapLabel: "Map for selecting location", DashboardUseMyLocation: "Use my location", DashboardPlace: "Place", DashboardLatitude: "Latitude", DashboardLongitude: "Longitude", DashboardWaterCharacteristics: "Water body characteristics", DashboardSurface: "Surface (m²)", DashboardDepth: "Depth (m)", DashboardStartDate: "Start date", DashboardEndDate: "End date", DashboardRunSimulation: "Run simulation", DashboardWeatherData: "Weather data", DashboardTemperature: "Temperature (°C)", DashboardRelativeHumidity: "Relative humidity (%)", DashboardWind: "Wind (km/h)", DashboardRadiationEvaporation: "Solar radiation and evaporation", DashboardResult: "Result", DashboardTotalEvaporation: "Total evaporation", DashboardEvaporatedVolume: "Evaporated volume", DashboardLinearMetersLowered: "Linear meters lowered", DashboardReadingsTitle: "Latest sensor readings", DashboardReadingsHelp: "Most recent data received from your MQTT sensors.", DashboardReadingsDevice: "Sensor", DashboardReadingsTemperature: "Temperature (°C)", DashboardReadingsTimestamp: "Received at", DashboardReadingsEmpty: "No readings yet. Add a sensor and start publishing data.", DashboardReadingsError: "Could not load sensor readings.",
	SensorGuideTitle: "Add an MQTT sensor", SensorGuideIntro: "Connect a sensor to send readings securely to your evap account.", SensorGuideConnection: "Connection data", SensorGuideEndpoint: "Broker host", SensorGuidePort: "Secure port", SensorGuideProtocol: "Protocol", SensorGuideTopic: "Topic", SensorGuideAuth: "Authentication", SensorGuideUsername: "Username", SensorGuidePassword: "Password", SensorGuideInterval: "Publication interval", SensorGuidePayload: "Message format", SensorGuidePayloadText: "Publish a JSON message with at least a numeric temperature and an ISO 8601 timestamp.", SensorGuideSteps: "Setup steps", SensorGuideStepOne: "Create the sensor as an AWS IoT Thing and assign it to your user account.", SensorGuideStepTwo: "Install the Thing certificate, private key, and the Amazon Root CA (download link above) on the sensor.", SensorGuideStepThree: "Publish over TLS using the topic and endpoint shown above.", SensorGuideBack: "Back to dashboard", SensorGuideDiagnosticsTitle: "Diagnostics", SensorGuideDiagnosticsHelp: "Check your added sensors and the messages received in the last 24 hours.", SensorGuideSensorsTitle: "Added sensors", SensorGuideSensorsEmpty: "No sensors added yet.", SensorGuideSensorsError: "Could not load sensors.", SensorGuideMessagesTitle: "Messages received (last 24h)", SensorGuideMessagesHelp: "Only messages from the last 24 hours are shown.", SensorGuideMessagesEmpty: "No messages received in the last 24 hours.", SensorGuideMessagesError: "Could not load received messages.", SensorGuideMessagesDevice: "Sensor", SensorGuideMessagesTemperature: "Temperature (°C)", SensorGuideMessagesTimestamp: "Received at",
}
...
var spanishMessages = map[string]string{
	...
	DashboardTitle: "Simulador de evaporación", DashboardGreeting: "Hola, %s", DashboardProfile: "perfil", DashboardSensorGuide: "MQTT", DashboardLogout: "Cerrar sesión", DashboardSystemStatus: "Estado del sistema", DashboardSimulation: "Simulación", DashboardTotalLoss: "Pérdida total", DashboardVolume: "Volumen", DashboardLoweredMeters: "Metros bajados", DashboardLocation: "Ubicación", DashboardLocationHelp: "Haz clic en el mapa, arrastra el marcador o utiliza tu ubicación actual.", DashboardMapLabel: "Mapa para seleccionar la ubicación", DashboardUseMyLocation: "Usar mi ubicación", DashboardPlace: "Lugar", DashboardLatitude: "Latitud", DashboardLongitude: "Longitud", DashboardWaterCharacteristics: "Características de la masa de agua", DashboardSurface: "Superficie (m²)", DashboardDepth: "Profundidad (m)", DashboardStartDate: "Fecha inicio", DashboardEndDate: "Fecha fin", DashboardRunSimulation: "Ejecutar simulación", DashboardWeatherData: "Datos meteorológicos", DashboardTemperature: "Temperatura (°C)", DashboardRelativeHumidity: "Humedad relativa (%)", DashboardWind: "Viento (km/h)", DashboardRadiationEvaporation: "Radiación solar y evaporación", DashboardResult: "Resultado", DashboardTotalEvaporation: "Evaporación total", DashboardEvaporatedVolume: "Volumen evaporado", DashboardLinearMetersLowered: "Metros lineales bajados", DashboardReadingsTitle: "Últimos envíos de sensor", DashboardReadingsHelp: "Datos más recientes recibidos de tus sensores MQTT.", DashboardReadingsDevice: "Sensor", DashboardReadingsTemperature: "Temperatura (°C)", DashboardReadingsTimestamp: "Recibido", DashboardReadingsEmpty: "Todavía no hay lecturas. Añade un sensor y empieza a publicar datos.", DashboardReadingsError: "No se pudieron cargar las lecturas del sensor.",
	SensorGuideTitle: "Añadir un sensor MQTT", SensorGuideIntro: "Conecta un sensor para enviar lecturas de forma segura a tu cuenta de evap.", SensorGuideConnection: "Datos de conexión", SensorGuideEndpoint: "Host del broker", SensorGuidePort: "Puerto seguro", SensorGuideProtocol: "Protocolo", SensorGuideTopic: "Topic", SensorGuideAuth: "Autenticación", SensorGuideUsername: "Usuario", SensorGuidePassword: "Contraseña", SensorGuideInterval: "Intervalo de publicación", SensorGuidePayload: "Formato del mensaje", SensorGuidePayloadText: "Publica un mensaje JSON con, como mínimo, una temperatura numérica y una marca de tiempo ISO 8601.", SensorGuideSteps: "Pasos de configuración", SensorGuideStepOne: "Crea el sensor como Thing de AWS IoT y asígnalo a tu cuenta de usuario.", SensorGuideStepTwo: "Instala en el sensor el certificado del Thing, la clave privada y la CA raíz de Amazon (enlace de descarga arriba).", SensorGuideStepThree: "Publica mediante TLS usando el topic y el endpoint indicados arriba.", SensorGuideBack: "Volver al panel", SensorGuideDiagnosticsTitle: "Diagnóstico", SensorGuideDiagnosticsHelp: "Consulta tus sensores añadidos y los mensajes recibidos en las últimas 24 horas.", SensorGuideSensorsTitle: "Sensores añadidos", SensorGuideSensorsEmpty: "Todavía no has añadido ningún sensor.", SensorGuideSensorsError: "No se pudieron cargar los sensores.", SensorGuideMessagesTitle: "Mensajes recibidos (últimas 24h)", SensorGuideMessagesHelp: "Solo se muestran los mensajes de las últimas 24 horas.", SensorGuideMessagesEmpty: "No se han recibido mensajes en las últimas 24 horas.", SensorGuideMessagesError: "No se pudieron cargar los mensajes recibidos.", SensorGuideMessagesDevice: "Sensor", SensorGuideMessagesTemperature: "Temperatura (°C)", SensorGuideMessagesTimestamp: "Recibido",
}
...
var frenchMessages = map[string]string{
	...
	DashboardTitle: "Simulateur d'évaporation", DashboardGreeting: "Bonjour, %s", DashboardProfile: "profil", DashboardSensorGuide: "MQTT", DashboardLogout: "Se déconnecter", DashboardSystemStatus: "État du système", DashboardSimulation: "Simulation", DashboardTotalLoss: "Perte totale", DashboardVolume: "Volume", DashboardLoweredMeters: "Mètres abaissés", DashboardLocation: "Emplacement", DashboardLocationHelp: "Cliquez sur la carte, faites glisser le marqueur ou utilisez votre position actuelle.", DashboardMapLabel: "Carte pour sélectionner l'emplacement", DashboardUseMyLocation: "Utiliser ma position", DashboardPlace: "Lieu", DashboardLatitude: "Latitude", DashboardLongitude: "Longitude", DashboardWaterCharacteristics: "Caractéristiques de la masse d'eau", DashboardSurface: "Surface (m²)", DashboardDepth: "Profondeur (m)", DashboardStartDate: "Date de début", DashboardEndDate: "Date de fin", DashboardRunSimulation: "Exécuter la simulation", DashboardWeatherData: "Données météorologiques", DashboardTemperature: "Température (°C)", DashboardRelativeHumidity: "Humidité relative (%)", DashboardWind: "Vent (km/h)", DashboardRadiationEvaporation: "Rayonnement solaire et évaporation", DashboardResult: "Résultat", DashboardTotalEvaporation: "Évaporation totale", DashboardEvaporatedVolume: "Volume évaporé", DashboardLinearMetersLowered: "Mètres linéaires abaissés", DashboardReadingsTitle: "Derniers envois de capteur", DashboardReadingsHelp: "Données les plus récentes reçues de vos capteurs MQTT.", DashboardReadingsDevice: "Capteur", DashboardReadingsTemperature: "Température (°C)", DashboardReadingsTimestamp: "Reçu à", DashboardReadingsEmpty: "Aucune lecture pour le moment. Ajoutez un capteur et commencez à publier des données.", DashboardReadingsError: "Impossible de charger les lectures du capteur.",
	SensorGuideBack: "Retour au tableau de bord", SensorGuideDiagnosticsTitle: "Diagnostic", SensorGuideDiagnosticsHelp: "Consultez vos capteurs ajoutés et les messages reçus au cours des dernières 24 heures.", SensorGuideSensorsTitle: "Capteurs ajoutés", SensorGuideSensorsEmpty: "Aucun capteur ajouté pour le moment.", SensorGuideSensorsError: "Impossible de charger les capteurs.", SensorGuideMessagesTitle: "Messages reçus (24h)", SensorGuideMessagesHelp: "Seuls les messages des dernières 24 heures sont affichés.", SensorGuideMessagesEmpty: "Aucun message reçu au cours des dernières 24 heures.", SensorGuideMessagesError: "Impossible de charger les messages reçus.", SensorGuideMessagesDevice: "Capteur", SensorGuideMessagesTemperature: "Température (°C)", SensorGuideMessagesTimestamp: "Reçu à",
}
